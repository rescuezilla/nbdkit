...
objects stored in Google Cloud Storage as disk images.

This plugin uses the Python Google Cloud Client Libraries
...
Learn about L<Google Service account keys|https://developers.google.com/workspace/guides/create-credentials#service-account>
and L<Application Default Credentials|https://cloud.google.com/docs/authentication/application-default-credentials>.
...
You can use the C<GOOGLE_APPLICATION_CREDENTIALS> environment variable to
provide the location of a credential JSON file.
...
interoperability mode. C<read> and C<write> work perfectly fine, while
C<trim> and C<zero> operations don't work in the general case: trimmed or
zeroed-out blocks cannot be deleted from the storage using the C<S3> plugin.

=head1 VERSION

C<nbdkit-gcs-plugin> first appeared in nbdkit 1.40.
