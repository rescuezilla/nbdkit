...
=head2 Show the amount of space used in a local VMDK file

This requires the L<nbdinfo(1)> utility from libnbd:

 $ nbdkit vddk -r /absolute/path/to/file.vmdk \
          --run 'nbdinfo --map --totals "$uri"'
  132055040   0.3%   0 data
 42817617920  99.7%   3 hole,zero

In this case the VMDK file is thin provisioned and almost entirely empty.
