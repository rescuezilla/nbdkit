...
command line, or L<nbdkit-readonly-filter(1)> To give selective access
to the server, use L<nbdkit-ip-filter(1)>.  To serve only a subset of
the plugin use L<nbdkit-offset-filter(1)> or
