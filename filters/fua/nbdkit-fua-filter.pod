...
=head1 SYNOPSIS
 ...
 nbdkit --filter=fua plugin [fuamode=MODE] [flush-on-close=true]
                            [plugin-args...]
...
use of FUA compared to a full flush.  Also it can be used to force a
flush when the client closes the connection.
...
=item B<flush-on-close=true>

(nbdkit E<ge> 1.44)

If set, call C<.flush> on the underlying plugin when the client closes
any NBD connection.  This can be used for misbehaving clients that do
not properly issue flush commands.  If the plugin does not support
flush then the filter will refuse to start.
...
Serve a file, flushing data to disk when the NBD client closes the
connection.  Note use of C<fuamode=pass> to stop the filter from
changing the behaviour of normal FUA requests:

 nbdkit --filter=fua file disk.img fuamode=pass flush-on-close=true
