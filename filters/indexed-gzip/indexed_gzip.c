...
// 1 megabyte default interval
static off_t span_in_bytes = 1024 * 1024;

/* The lock prevents concurrent access during index loading. */
static pthread_mutex_t lock = PTHREAD_MUTEX_INITIALIZER;

static void
indexed_gzip_unload(void)
{
...
#define indexed_gzip_config_help \
  "gzip-index-path=<PATH>                The path to the complete gzip index file (created by gztool).\n" \
  "span=<PATH>                           The number of bytes between index points.\n"
...
indexed_gzip_prepare(nbdkit_next *next, void *handle, int readonly)
{
    ...
    if (input_file_fp == NULL) {
        ...
        if (deflate_index_serialize(h->index, new_index_file_fp) < 0) {
            nbdkit_error(FILTER_NAME ": failed to write the index to file\n", len);
            return -1;
        }
                              ...
                              uint32_t flags, int *err)
{
    ...
    if (len < 0) {
        ...
        default:
            //
            *err = EIO;
            nbdkit_error(FILTER_NAME ": error %d while extracting value\n", len);
        }
        return -1;
    }
}

/* Set thread model to single-threaded for safety with index operations. */
static int
indexed_gzip_thread(int next_thread_model)
{
    // TODO: Drop thread model restrictions
    /* Force single-threaded operation to avoid race conditions in index access
     * and zlib decompression state. */
...
static struct nbdkit_filter filter = {
    ...
    .thread_model       = indexed_gzip_thread,
};
