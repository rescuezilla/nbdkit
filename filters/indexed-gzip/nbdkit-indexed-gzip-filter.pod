...
C<nbdkit-indexed-gzip-filter> is a filter for nbdkit that provides fast and
efficient random-access to gzip-compressed data.

The standard C<nbdkit-gzip-filter> decompresses the entire archive to a
...
"split" plugins and the "truncate" filter) don't have any such overbearing
penalty around system resources.

With this plugin, the entire archive still needs to be decompressed once,
...
decompress until reaching the requested byte (with the trade-off of a larger
index file).
...
C<nbdkit-indexed-gzip-filter> first appeared in nbdkit 1.46.
...
gztool.c based on zran.c by zlib author Mark Adler
