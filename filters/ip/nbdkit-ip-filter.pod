...
against the wildcard C<GLOB>.  See L<nbdkit_peer_tls_dn(3)> and
L<nbdkit-tls(1)> for further information.

Because DNs contain commas (C<,>) but comma is also normally used to
split allow/deny list elements, this filter has a special exception
where if the first 3 characters of the allow or deny parameter match
C<"dn:"> then the parameter is I<not> split.  This lets you match on
DNs naturally using:

 nbdkit --filter=ip allow=dn:"*,O=BigCo,*" allow=dn:"*,C=US" [...]

B<Note:> If your allow or deny patterns contain any DN rules then
I<all> filtering has to be done later in the
L<connection lifecycle|nbdkit-plugin(3)/Callback lifecycle> since we
must wait until TLS negotiation has been completed.  This makes
filtering more expensive and potentially easier to cause DoS attacks.
You should try to combine DN rules with early filtering outside nbdkit
like using a firewall or VPN.
...
In S<nbdkit E<le> 1.40>, connections from non-IP, non-Unix, non-vsock
sockets (whatever that would be) were allowed unfiltered.  Unknown
socket families are now denied.
