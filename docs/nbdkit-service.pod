...
or can be sent to a file (I<--log=/path>).

The default, if I<--log> is not specified on the command line, or if
you use I<--log=default>, is to send error messages to stderr, unless
nbdkit forks into the background in which case they are sent to
syslog.
...
=item Messages are appended to a named file:

Using I<--log=/path>.  The parameter must be an absolute path.  The
