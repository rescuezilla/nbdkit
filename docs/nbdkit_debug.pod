...
These functions send the debug message to either stderr or the system
logs depending on the current server state and the user-specified
nbdkit I<--log> option.  Debug messages are only emitted when the
server is in verbose mode (nbdkit I<-v> option)
