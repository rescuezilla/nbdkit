...
nbdkit 1.40.0 was released on B<XXX XXX 2024>.
...
would not necessarily deny every connection.  This has been change in
...
rotational property of a plugin (whether it acts like a spinning hard
disk, or RAM / flash storage).
...
When generating the NBD URI with TLS enabled, append
