...
defining this callback.  The C<next_thread_model> parameter passed in
is the thread model of the underlying layer (for a plugin, its thread
model, for a filter, what it returned from C<.thread_model>).
