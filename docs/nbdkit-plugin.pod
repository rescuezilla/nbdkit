...
If conversion was not possible, this calls L<nbdkit_error(3)> and returns
...
If the path resolution was not possible, this calls L<nbdkit_error(3)>
and returns C<NULL>.
...
requested, or -1 after calling L<nbdkit_error(3)> if there is no point in
...
On error, L<nbdkit_error(3)> is called and the call returns C<NULL>.
...
C<.open>), L<nbdkit_error(3)> is called and the call returns C<-1>.
...
On success this returns C<0>.  On error, L<nbdkit_error(3)> is called and
...
L<nbdkit_error(3)> is called and this call returns C<-1>.
...
On success this returns the user ID.  On error, L<nbdkit_error(3)> is
...
unsupported protocol), L<nbdkit_error(3)> is called and this call returns
C<NULL>.
