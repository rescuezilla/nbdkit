...
=head1 EXPORT-SAFE FILTERS

When a plugin supports NBD export names, it may serve different
content (eg. different disk images) on each export.  Some nbdkit
filters do not work properly in this case.
