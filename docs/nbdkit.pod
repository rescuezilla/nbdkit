...
system log (I<--log=syslog>), discard them completely (I<--log=null>,
not recommended for normal use), or append them to a file (which must
be an absolute path).

The default (if I<--log> is not used, or if you use I<--log=default>)
is: send error messages to stderr, unless nbdkit forks into the
...
If you need to pass a large number of key=value parameters, you can
place them in an external file, for example:
...
The pathname after the C<@> must contain at least one C</> character
(this is to avoid compatibility problems with some existing plugins),
so if the file is the current directory as in the example above, use
C<@./filename>

There must be one parameter per line.  Lines beginning with C<#> and
blank lines are ignored.

This only works for C<key=value> plugin & filter parameters (not for
the plugin name or I<--options>).  You can use multiple C<@PATH>
parameters.

You or use C<@PATH> recursively in another C<@PATH> file.  The path in
that case is taken relative to the containing file.
...
L<nbdkit-tracing(3)> — Adding user statically defined tracing (USDT)
probes to nbdkit.
